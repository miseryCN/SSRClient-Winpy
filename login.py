...
def api_login(username,password):
    ...
    if login_info['status'] == 'success':  # status判断登录成功
        ...
        upload_transfer = user_info['u']
        download_transfer = user_info['d']
        last_use = user_info['t']
        link = login_info['data']['link']  # 订阅链接
        conn_info = subscribe_link(link)    #解析订阅链接
        # 对时间进行格式化
        ...
        info_dic = {
            ...
            'transfer_enable' : transfer_enable,    #可用流量
            'used_transfer' : used_transfer,        #已用流量
            'last_use' : last_use,                  #最后使用时间
            'enable' : user_info['enable'],         #是否可用
            'balance' : user_info['balance'],       #余额
            'score' : user_info['score'],           #积分
            'enable_time' : user_info['enable_time'],   #注册时间
            'expire_time' : user_info['expire_time'],   #过期时间
            'status' : user_info['status'],             #用户状态 封禁/正常/未激活
            'password' : user_info['passwd'],           #连接密码
            'conn_info' : conn_info,                    #连接信息
            'link' : link                               #订阅链接
        }
        return info_dic
    else:
